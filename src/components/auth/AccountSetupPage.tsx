...
const AccountSetupPage: React.FC = () => {
    ...
    return (
        <div className="min-h-screen w-full bg-gradient-to-br from-golden-700 to-golden-950 text-white flex flex-col justify-center items-center p-4 relative">
            ...
            <div className="w-full max-w-md bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-2xl p-8 sm:p-10 z-10">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                     ...
                     <div>
                        <label className="block text-golden-200 text-sm font-bold mb-2" htmlFor="role">What is your primary role?</label>
                        <select 
                            id="role" 
                            value={role} 
                    ...
                    <div className="pt-2">
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-white text-golden-800 font-bold py-3 px-4 rounded-lg hover:bg-golden-100 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                        >
            ...
            <style>{`.input-auth {
                @apply appearance-none border rounded-lg w-full py-3 px-4 bg-white/20 text-white placeholder-golden-200/70 border-white/30 leading-tight focus:outline-none focus:ring-2 focus:ring-golden-400;
            }`}</style>
        </div>
    );
};
