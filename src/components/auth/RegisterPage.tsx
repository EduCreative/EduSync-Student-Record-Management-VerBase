...
const RegisterPage: React.FC<RegisterPageProps> = ({ onSwitchToLogin }) => {
    ...
    if (success) {
        return (
            <div className="min-h-screen bg-secondary-100 dark:bg-secondary-900 flex flex-col justify-center items-center p-4">
                <div className="w-full max-w-md bg-white dark:bg-secondary-800 rounded-2xl shadow-2xl p-6 sm:p-8 text-center">
                    ...
                    <button onClick={onSwitchToLogin} className="mt-6 btn btn-primary w-full">
                        Back to Login
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen w-full bg-white dark:bg-secondary-950 lg:grid lg:grid-cols-2">
            ...
            <div className="flex items-center justify-center p-6 sm:p-12 w-full lg:py-0 bg-secondary-100 dark:bg-secondary-900">
                 <div className="w-full max-w-md">
                    ...
                    <form onSubmit={handleRegister} className="space-y-4" noValidate>
                        <div>
                            ...
                            <div className="relative">
                                <UserIcon className="pointer-events-none w-5 h-5 absolute top-1/2 transform -translate-y-1/2 left-3 text-secondary-400"/>
                                <input id="name" type="text" value={name} onChange={handleInputChange(setName, 'name')} className="input-field pl-10" required />
                            </div>
                        ...
                        <div>
                            <label className="block text-secondary-700 dark:text-secondary-300 text-sm font-bold mb-2" htmlFor="email">Email Address</label>
                             <div className="relative">
                                ...
                                <input id="email" type="email" value={email} onChange={handleInputChange(setEmail, 'email')} className="input-field pl-10" required />
                            </div>
                        ...
                        <div>
                            <label className="block text-secondary-700 dark:text-secondary-300 text-sm font-bold mb-2" htmlFor="password">Password</label>
                             <div className="relative">
                                ...
                                <input id="password" type="password" value={password} onChange={handleInputChange(setPassword, 'password')} className="input-field pl-10" required />
                            </div>
                    ...
                    <p className="text-center text-secondary-500 dark:text-secondary-400 text-sm mt-6">
                        Already have an account?{' '}
                        <button onClick={onSwitchToLogin} className="font-semibold text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300">
                            Sign In
                        </button>
                    </p>
                </div>
