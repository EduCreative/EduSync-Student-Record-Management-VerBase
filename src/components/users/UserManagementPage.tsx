...
export const UserManagementPage: React.FC<UserManagementPageProps> = ({ payload }) => {
    ...
    const canAddUser = hasPermission(Permission.CAN_MANAGE_USERS);
    const canDeleteUser = hasPermission(Permission.CAN_DELETE_USERS);
    ...
    const skeletonColumns = [{width: '35%'}, {width: '20%'}, {width: '15%'}, {width: '15%'}, {width: '15%'}];
    ...
    return (
        <>
            ...
            {canDeleteUser && (
                <Modal isOpen={!!userToDelete} onClose={() => setUserToDelete(null)} title="Confirm User Deactivation">
                    <div>
                        <p className="text-sm text-secondary-600 dark:text-secondary-400">
                            Are you sure you want to deactivate the user{' '}
                            <strong className="text-secondary-800 dark:text-secondary-200">{userToDelete?.name}</strong>? Their account will be set to 'Inactive' and they will no longer be able to log in.
                        </p>
                        <div className="mt-6 flex justify-end space-x-3">
                            <button type="button" onClick={() => setUserToDelete(null)} className="btn-secondary">Cancel</button>
                            <button type="button" onClick={handleDeleteUser} className="btn-danger">Deactivate</button>
                        </div>
                    </div>
                </Modal>
            )}
            ...
            <div className="space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    <h1 className="text-3xl font-bold text-secondary-900 dark:text-white">User Accounts</h1>
                    {canAddUser && (
                ...
                <div className="p-4 bg-white dark:bg-secondary-800 rounded-lg shadow-md">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        {isOwnerGlobalView && (
                            <select value={schoolFilter} onChange={e => setSchoolFilter(e.target.value)} className="input-field lg:col-span-4">
                                    ...
                                    <tbody>
                                        {paginatedUsers.map(user => (
                                            <tr key={user.id} className="bg-white dark:bg-secondary-800 border-b dark:border-secondary-700 hover:bg-secondary-50 dark:hover:bg-secondary-700/50">
                                                ...
                                                <td className="px-6 py-4">
                                                    <Badge color={user.status === 'Active' ? 'green' : user.status === 'Pending Approval' ? 'yellow' : 'red'}>
                                                        {user.status}
                                                    </Badge>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center space-x-4">
                                                        {canAddUser && <button onClick={() => handleOpenModal(user)} className="font-medium text-primary-600 dark:text-primary-500 hover:underline">Edit</button>}
                                                        {canDeleteUser && user.id !== currentUser?.id && <button onClick={() => setUserToDelete(user)} className="font-medium text-red-600 dark:text-red-500 hover:underline">Deactivate</button>}
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
