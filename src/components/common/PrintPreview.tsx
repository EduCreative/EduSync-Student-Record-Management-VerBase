...
const PrintPreview: React.FC = () => {
    ...
    const handlePrint = () => {
        ...
        const stylesAndScripts = document.querySelectorAll(
            'style, link[rel="stylesheet"], script[src="https://cdn.tailwindcss.com"]'
        );
        
        // Find the tailwind config script
        const tailwindConfigScript = Array.from(document.scripts).find(
          (script) => script.textContent?.includes('tailwind.config')
        );
        ...
        if (tailwindConfigScript) {
            headContent += tailwindConfigScript.outerHTML;
        }
