        ...
        <Modal isOpen={isOpen} onClose={onClose} title={classToEdit ? 'Edit Class' : 'Add New Class'}>
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                <div>
                    ...
                    <input type="text" name="name" id="name" value={formData.name} onChange={e => setFormData(p => ({...p, name: e.target.value}))} className="w-full input-style" />
                    {error && <p className="text-red-500 text-xs mt-1">{error}</p>}
                </div>
                <div>
                    <label htmlFor="teacherId" className="input-label">Assign Teacher</label>
                    <select name="teacherId" id="teacherId" value={formData.teacherId} onChange={e => setFormData(p => ({...p, teacherId: e.target.value}))} className="w-full input-style">
