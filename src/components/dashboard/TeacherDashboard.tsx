...
const TeacherDashboard: React.FC = () => {
    ...
    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-shine">Teacher's Dashboard</h1>
                <p className="text-secondary-500 dark:text-secondary-400">Welcome, {user.name}. Here are your classes and tasks.</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="animate-in" style={{ animationDelay: '100ms' }}><StatCard title="Assigned Classes" value={assignedClasses.length.toString()} color="bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300" icon={<BookOpenIcon />} /></div>
                <div className="animate-in" style={{ animationDelay: '200ms' }}><StatCard title="Total Students" value={studentsInMyClasses.length.toString()} color="bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-300" icon={<UsersIcon />} /></div>
                <div className="animate-in" style={{ animationDelay: '300ms' }}><StatCard title="Average Attendance" value={stats.averageAttendance} color="bg-green-100 dark:bg-green-900/50 text-green-600 dark:text-green-300" icon={<CheckCircleIcon />} /></div>
                <div className="animate-in" style={{ animationDelay: '400ms' }}><StatCard title="Subjects Taught" value={stats.subjectsTaught} color="bg-yellow-100 dark:bg-yellow-900/50 text-yellow-600 dark:text-yellow-300" icon={<ClipboardListIcon />} /></div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="animate-in" style={{ animationDelay: '500ms' }}><DoughnutChart title="Overall Attendance Summary" data={attendanceSummary} /></div>
                <div className="animate-in" style={{ animationDelay: '600ms' }}><BarChart title="Latest Exam Performance (Avg %)" data={subjectPerformance} multiColor={true} /></div>
            </div>
                    ...
                    {assignedClasses.map(c => (
                        <div key={c.id} className="p-4 border dark:border-secondary-700 rounded-lg flex justify-between items-center">
                            ...
                            <div className="flex space-x-2">
                                <button className="px-3 py-1 text-sm font-medium text-primary-700 bg-primary-100 dark:bg-primary-900 dark:text-primary-200 rounded-md hover:bg-primary-200 active:scale-95">Take Attendance</button>
                                <button className="px-3 py-1 text-sm font-medium text-green-700 bg-green-100 dark:bg-green-900 dark:text-green-200 rounded-md hover:bg-green-200 active:scale-95">Enter Results</button>
                            </div>
                        </div>
                    ))}
